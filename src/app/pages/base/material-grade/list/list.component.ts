...
import { MaterialGradeResponse } from '@app/shared/models/base.interface';
declare var $: any;
...
export class ListComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  loadTable(): void {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        that.materialgradeSvc.getAll(dataTablesParameters).subscribe((resp) => {
  ...
  onEdit(data): void {
    ...
    this.router.navigate(['base/material-grade/edit'], navigationExtras);
  }

  onDelete(materialgradeId: number): void {
    if (window.confirm('Do you really want remove this data')) {
      this.materialgradeSvc
        .delete(materialgradeId)
        .pipe(takeUntil(this.destroy$))
        .subscribe((res: MaterialGradeResponse) => {
          if (res.code == 201) {
            this.rerender();
          }
          // this.branchSvc.getAll().subscribe((branch) => {
