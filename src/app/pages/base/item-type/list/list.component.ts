...
import { ItemTypeResponse } from '@app/shared/models/base.interface';
declare var $: any;
...
export class ListComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  constructor(
    ...
    private itemtypehSvc: ItemTypeService,
  ...
  loadTable(): void {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        that.itemtypehSvc.getAll(dataTablesParameters).subscribe((resp) => {
  ...
  onEdit(data): void {
    ...
    this.router.navigate(['base/item-type/edit'], navigationExtras);
  }

  onDelete(itemtypeId: number): void {
    if (window.confirm('Do you really want remove this data')) {
      this.itemtypehSvc
        .delete(itemtypeId)
        .pipe(takeUntil(this.destroy$))
        .subscribe((res: ItemTypeResponse) => {
          if (res.code == 201) {
            this.rerender();
          }
          // this.branchSvc.getAll().subscribe((branch) => {
