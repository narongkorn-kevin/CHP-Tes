...
import { SpareTypeResponse } from '@app/shared/models/base.interface';
...
export class ListComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  loadTable(): void {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        that.sparetypeSvc.getAll(dataTablesParameters).subscribe((resp) => {
  ...
  onEdit(data): void {
    ...
    this.router.navigate(['base/spare-type/edit'], navigationExtras);
  }

  onDelete(sparetypeId: number): void {
    if (window.confirm('Do you really want remove this data')) {
      this.sparetypeSvc
        .delete(sparetypeId)
        .pipe(takeUntil(this.destroy$))
        .subscribe((res: SpareTypeResponse) => {
          if (res.code == 201) {
            this.rerender();
          }
          // this.branchSvc.getAll().subscribe((branch) => {
