...
import { MaterialTypeResponse } from '@app/shared/models/base.interface';
declare var $: any;
...
export class ListComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  loadTable(): void {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        that.materialtypeSvc.getAll(dataTablesParameters).subscribe((resp) => {
  ...
  onEdit(data): void {
    ...
    this.router.navigate(['base/material-type/edit'], navigationExtras);
  }

  onDelete(warehouseId: number): void {
    if (window.confirm('Do you really want remove this data')) {
      this.materialtypeSvc
        .delete(warehouseId)
        .pipe(takeUntil(this.destroy$))
        .subscribe((res: MaterialTypeResponse) => {
          if (res.code == 201) {
            this.rerender();
          }
          // this.branchSvc.getAll().subscribe((branch) => {
