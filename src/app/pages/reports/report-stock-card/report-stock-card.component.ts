...
declare const M: any;
@Component({
  selector: 'app-report-stock-card',
  templateUrl: './report-stock-card.component.html',
  styleUrls: ['./report-stock-card.component.scss']
})
export class ReportStockCardComponent implements OnInit {
  ...
  loadTable(dataTablesParameters: FormGroup): void {
    ...
    param.item_type_id = [dataTablesParameters.value.item_type_id];

    if(param.item_type_id ==''){
      Swal.fire({title:'Warning',text:'Please select Item type!',icon:'warning'}); 
      return; 
    }
  ...
  getRowItemType(item_type_id){
    let res;
    if(item_type_id) res = this.itemTypeRow.filter((item)=> item.id == item_type_id)
    if(res) return res[0];
  }
