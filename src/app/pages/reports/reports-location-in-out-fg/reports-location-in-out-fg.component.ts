...
@Component({
  selector: 'app-reportslocation-in-out-fg',
  templateUrl: './reports-location-in-out-fg.component.html',
  styleUrls: ['./reports-location-in-out-fg.component.scss']
})
export class ReportslocationInOutFgComponent implements OnInit {
  ...
  getLocation(e) {
    ...
    this.services.getLocation({ warehouse_id: warehouse_id }).subscribe((resp) => {
      this.locationRow = resp.data;
    });
  }
  ...
  getWareHouse() {
    this.services.getWareHouse().subscribe((resp) => {
      this.wareHouseRow = resp.data;
    });
  }
  ...
  loadTable(dataTablesParameters: FormGroup): void {
    ...
    param.location_id = [param.location_id];
    this.services.getAll(param).subscribe((resp) => {
      $('#example').DataTable().destroy();
      this.dataRow = resp.data;
      if (this.dataRow.length < 1) { alert('Data not found!'); } else {
        let msgTop = '';
        let locName = this.locationRow.filter((obj) => { if (obj.id == param.location_id) { return obj; } })
        msgTop = `Location ${locName[0].name}`;
